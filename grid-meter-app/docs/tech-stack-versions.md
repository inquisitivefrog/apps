...
| Apache JMeter | 5.6.3 | Confirmed still current stable Aug 2026 (last real release Jan 2024; Java 17+ recommended, satisfied by this project's Java 25). Runs natively on the host (`brew install jmeter`), not containerized — see `architecture.md`'s resource budget notes. Test plans in `load-tests/`. |
