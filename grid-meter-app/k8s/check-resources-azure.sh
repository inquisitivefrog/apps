#!/usr/bin/env bash
# Kubernetes-layer counterpart to terraform/azure/check-resources-azure.sh - confirms every object
# deploy-azure.sh is supposed to create actually exists and is healthy on the real cluster, queried
# directly via kubectl, not trusted from `kubectl rollout status`'s own "successfully rolled out"
# message alone. Intended to run right after deploy-azure.sh, before a demo or before
# teardown-azure.sh. Mirrors k8s/check-resources-aws.sh's/check-resources-gcp.sh's structure
# exactly - same object names across every target, since deploy-azure.sh creates them identically.
# One real difference, already known rather than left to be rediscovered: the default
# StorageClass here is managed-csi-xfs (storageclass-azure.yaml), not gp3/pd-balanced-xfs.
#
# NOT yet live-tested against a real cluster as of 2026-09-23, same caveat deploy-azure.sh/
# teardown-azure.sh carry - only syntax-checked (bash -n) this pass.
set -uo pipefail

PASS=0
FAIL=0

# check <label> <expect> <command...> - same as k8s/check-resources-aws.sh/check-resources-gcp.sh:
# requires both a zero exit code and non-empty trimmed output before comparing to <expect> ("any"
# skips the comparison) - a bare output-presence check alone would misread kubectl's own
# "Error from server (NotFound): ... not found" text (itself non-empty) as a real value, the exact
# false-PASS AWS's identical script found the hard way (2026-09-18).
check() {
...
check "managed-csi-xfs StorageClass is default" "true" kubectl get storageclass managed-csi-xfs -o jsonpath='{.metadata.annotations.storageclass\.kubernetes\.io/is-default-class}'
echo
